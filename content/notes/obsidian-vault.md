...
The **Memory Layer** (`ObsidianWriter`) writes new notes to the vault after task completion, passing every write through the 4-gate Memory Write Validator first. The **session_to_vault.py** script parses each coding session's transcript, uses Claude Haiku to write a narrative summary, and saves it as a `sessions/YYYY-MM-DD.md` file. The **vault_sync.py** script reads the actual codebase state and updates the `status:` field in each component's vault note to accurately reflect whether the component is built, partial, or planned.
